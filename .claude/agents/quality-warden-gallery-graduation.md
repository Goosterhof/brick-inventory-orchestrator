...
| SOP G-3 (Doc Accuracy): when verifying a "shipped/in-progress" Pulse claim, confirm the PR's actual merge state via `git log --grep` AND that the cited CI gate's most recent failures (if any) were caused by that gate vs an unrelated step | 2026-05-29 | 2026-05-29-gallery-pulse-refresh | The `arch-cleanup-sweep` CI run failed on "Lint commit messages" while its Stryker mutation step passed; a name-only read would have mis-recorded the mutation gate as unstable and falsely blocked the v2 promotion assessment. |
| SOP G-4 / S-cross: when re-measuring a timing-based guard (collect/test guard), run it ≥2 times and record the range, not a single value — collect deltas are baseline-order-sensitive | 2026-05-29 | 2026-05-29-gallery-pulse-refresh | `AboutPage.spec.ts` swung from below-the-400ms-warning-floor to a 932ms delta across two runs of the same `npm run test:coverage` command (cold baseline = first file in its worker thread to pay the import chain). The Pulse's recorded "520ms" was one point on the swing, not a stable value. |
| SOP G-3 (Doc Accuracy): when an Active Concern is marked Closed, scan the Overall Health narrative for stale inline references to it — separately-maintained Pulse sections drift out of sync | 2026-05-29 | 2026-05-29-gallery-pulse-refresh | The `PartsPage` collect-guard violation closed 2026-05-27 (PR #119) and the Active Concerns section recorded the closure, but the Overall Health narrative still led with it as "the loudest active medium" on 2026-05-29. |
