...
6. **PrePushPermitGate posture for this delivery.** Diff is 9 files modified/created (under 20-file threshold) and ~1170 lines added (over the 500-line threshold). I expected the gate to fire and tested AC #54 against it. **Outcome: the PrePushPermitGate did not fire because my changes touched only `frontend/` and `.claude/` — no `backend/` paths.** Per `.githooks/pre-push`, the backend gate (which embeds the permit check) only runs when the pushed range touches `backend/`. The frontend pre-push hook is the Husky `type-check → knip → test:coverage → build` chain — no permit-gate equivalent. So the dispatch's dual-mode question about AC #54 vs PrePushPermitGate was not actually tested by this delivery — the gate didn't run. Recording for the ADR-0028 dual-mode amendment thinking: the *frontend* pre-push lane has no permit-gate today, so frontend deliveries can always close WOs in the same commit without risk of the dual-mode escape path. **Did not use `--no-verify`.** |
