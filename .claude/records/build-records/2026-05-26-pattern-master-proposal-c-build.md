...
| Modified | `frontend/src/tests/unit/apps/showcase/components/BrickShapes.spec.ts` | Replaced 7-test scaffold with 18 tests covering rendering, slider reactivity, idle→hover→press→release→leave traversal, badge text, and reduced-motion live updates. Mocked the seven `LegoXxx.vue` imports to keep collect-time inside the project baseline. |
...
| test:coverage | Pass | 100% statements (1445/1445), 100% branches (1118/1118), 100% functions (422/422), 100% lines (1344/1344). Test guard warned at coverage-mode 2x threshold on `SetsOverviewPage.spec.ts` (3808ms) and `ComponentGallery.spec.ts` (2210ms) — both pre-existing files, not touched by this delivery. My BrickShapes.spec.ts measured at 587ms after mocking the seven LegoXxx imports; useBrickPickup.spec.ts at 181ms. |
