...
```
shift: 11                 # current shift number (claimed at clock-in; monotonic across runs)
rotation: 6               # patrol dimension rotation counter (NOTE: named-workflow args don't reach the script — pin TODAY/ROTATION in a scratchpad script copy; standing workaround since shift 003.)
...
consecutive_dry: 2        # shifts in a row with nothing filed and nothing built (2 → doors close)
