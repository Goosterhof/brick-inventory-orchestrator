...
describe('PartsMissingPage', () => {
    ...
    it('renders the page title from translation keys', async () => {
        ...
        // Act
        const wrapper = shallowMount(PartsMissingPage, {global: {stubs: {FormField: false, TextInput: false}}});
        await flushPromises();

        // Assert
        expect(wrapper.findComponent({name: 'PageHeader'}).props('title')).toBe('parts.missingTitle');
    });
    ...
    it('fetches the master shopping list on mount', async () => {
        ...
        // Act
        shallowMount(PartsMissingPage, {global: {stubs: {FormField: false, TextInput: false}}});
        await flushPromises();
    ...
    it('shows the empty state when there are no entries and no unknown sets', async () => {
        ...
        // Assert
        expect(wrapper.findComponent({name: 'EmptyState'}).props('message')).toBe('parts.missingEmpty');
    });
    ...
    describe('sort', () => {
        ...
        it('sorts by part name when the name chip is clicked', async () => {
            ...
            const nameChip = wrapper
                ...
                .find((c) => c.text() === 'parts.missingSortName');
            emit(nameChip, 'click');
            await flushPromises();
        ...
        it('sorts by color name when the color chip is clicked (empty strings sort first)', async () => {
            ...
            const colorChip = wrapper
                .findAllComponents({name: 'FilterChip'})
                .find((c) => c.text() === 'parts.missingSortColor');
