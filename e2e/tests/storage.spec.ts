...
test.describe("Storage", () => {
    ...
    test("can add a storage location", async ({ page }) => {
        ...
        await expect(
            page.getByRole("heading", { name: "Add storage" }),
        ).toBeVisible();
        ...
        await expect(
            page.getByRole("heading", { name: "Drawer A" }),
        ).toBeVisible();
    });
    ...
    test("can view storage details", async ({ page }) => {
        ...
        await expect(
            page.getByRole("heading", { name: "Drawer B" }),
        ).toBeVisible();
    ...
    test("can edit a storage location", async ({ page }) => {
        ...
        await expect(
            page.getByRole("heading", { name: "Edit storage" }),
        ).toBeVisible();
        ...
        await page.waitForURL(
            (url) => url.pathname === `/storage/${response.data.id}`,
        );
        await expect(
            page.getByRole("heading", { name: "Drawer C Renamed" }),
        ).toBeVisible();
    });
    ...
    test("storage overview shows tree structure", async ({ page }) => {
        ...
        // Create parent
        const parent = await api.post<{ data: { id: number } }>(
            "/storage-options",
            {
                name: "Cabinet",
                description: null,
